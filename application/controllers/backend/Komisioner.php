...
class Komisioner extends CI_Controller{
	function __construct(){
		...
		$this->load->model('backend/Komisioner_model','komisioner_model');
		$this->load->library('upload');
		$this->load->helper('text');
	}

	function index(){
		$x['data'] = $this->komisioner_model->get_all_post();
		$this->load->view('backend/v_komisioner',$x);
	}

	function add_new(){
		$x['tag']	   = $this->tag_model->get_all_tag();
		$x['category'] = $this->category_model->get_all_category();
		$this->load->view('backend/v_add_komisioner',$x);
	}
	...
	function get_edit(){
		...
		$x['tag']	   = $this->tag_model->get_all_tag();
		$x['category'] = $this->category_model->get_all_category();
		$x['data'] = $this->komisioner_model->get_post_by_id($post_id);
		$this->load->view('backend/v_edit_komisioner',$x);
	}

	function publish(){
		$config['upload_path'] = './assets/komisioner/';
	    ...
	    if(!empty($_FILES['filefoto']['name'])){
	        if ($this->upload->do_upload('filefoto')){
	            ...
	            $config['source_image']='./assets/komisioner/'.$img['file_name'];
	            $config['create_thumb']= FALSE;
	            $config['maintain_ratio']= TRUE;
	            $config['quality']= '60%';
	            $config['width']= 500;
	            $config['height']= 500;
	            $config['new_image']= './assets/komisioner/'.$img['file_name'];
			...
			$description=$this->input->post('contents',TRUE);
				...
				$this->komisioner_model->save_post($title,$slug,$jabatan,$email,$image,$description);
				echo $this->session->set_flashdata('msg','success');
				redirect('backend/komisioner');
			}else{
	    ...
	    if(!empty($_FILES['filefoto']['name'])){
	        if ($this->upload->do_upload('filefoto')){
	         ...
	         //   $this->_create_thumbs($img['file_name']);
				...
				$query = $this->db->get_where('tbl_komisioner', array('post_slug' => $praslug));
				if($query->num_rows() > 1){
					$uniqe_string = rand();
					$slug = $praslug.'-'.$uniqe_string;
				}else{
				...
				$this->komisioner_model->edit_post_with_img($id,$title,$jabatan,$email,$slug,$image,$description);
			...
			$this->komisioner_model->edit_post_no_img($id,$title,$jabatan,$email,$slug,$description);
			echo $this->session->set_flashdata('msg','info');
			redirect('backend/komisioner');
		}
	...
	function delete(){
		...
		$this->komisioner_model->delete_post($post_id);
