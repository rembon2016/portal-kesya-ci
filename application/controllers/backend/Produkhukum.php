...
class Produkhukum extends CI_Controller{
	...
	//	$this->load->model('backend/Tag_model','tag_model');
		$this->load->model('backend/Category_produk_model','category_model');
		$this->load->model('backend/Produkhukum_model','produkhukum_model');
		$this->load->library('upload');
		$this->load->helper('text');
	}

	function index(){
		$x['data'] = $this->produkhukum_model->get_all_post();
		$this->load->view('backend/v_produkhukum',$x);
	}

	function add_new(){

		$x['category'] = $this->category_model->get_all_category();
		$this->load->view('backend/v_add_produkhukum',$x);
	}
	...
	function get_edit(){
		...
		$x['category'] = $this->category_model->get_all_category();
		$x['data'] = $this->produkhukum_model->get_post_by_id($post_id);
		$this->load->view('backend/v_edit_post',$x);
	}

	function publish(){
		$config['upload_path'] = './assets/file/produkhukum/';
        $config['allowed_types'] = 'pdf|doc|docx|pptx|gif|jpg|png|jpeg|bmp';
	    ...
	    if(!empty($_FILES['filefoto']['name'])){
	        if ($this->upload->do_upload('filefoto')){
	            ...
	            // $config['source_image']='./assets/file/produkhukum/'.$img['file_name'];
	            // $config['create_thumb']= FALSE;
	            // $config['maintain_ratio']= FALSE;
	            // $config['quality']= '60%';
	            // $config['width']= 500;
	            // $config['height']= 320;
	            // $config['new_image']= './assets/file/produkhukum/'.$img['file_name'];
				...
				$contents = $this->input->post('description');
				$category = $this->input->post('category',TRUE);
				$image_link = $this->input->post('link',TRUE);

				$preslug  = strip_tags(htmlspecialchars($this->input->post('title',TRUE),ENT_QUOTES));
				$string   = preg_replace('/[^a-zA-Z0-9 \&%|{.}=,?!*()"-_+$@;<>\']/', '', $preslug);
				$trim     = trim($string);
				$praslug  = strtolower(str_replace(" ", "-", $trim));
				$query = $this->db->get_where('tbl_produk_hukum', array('post_slug' => $praslug));
				...
				$this->produkhukum_model->save_post($title,$description,$image,$image_link,$category,$slug);

				echo $this->session->set_flashdata('msg','success');
				redirect('backend/Produkhukum');
			}else{
				...
				$this->produkhukum_model->save_post($title,$description,$image,$image_link,$category,$slug);
			//	var_dump($title,$description,$image,$image_link,$category,$slug);die();
